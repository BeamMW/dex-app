  ...
  public static generateShaderApi<ShaderActions>(cid: string, methodsList: Array<IMethod<ShaderActions>>) {
    ...
    for (const method of methodsList) {
      ...
      const generatedMethod: MethodReadable<ShaderActions> = new methodClass(
        generator.getCid(),
        //for compability with old contracts
        method?.role,
        method.action,
      ...
      registeredMethods = {
        ...registeredMethods, ...{
          /* for snakeToCamel transform */
          ...{ [generator.snakeToCamel(/* For role-based contract compability */(method.role ? method.role + "_" : "") + method.action)]: generatedMethod }
        }
      }
